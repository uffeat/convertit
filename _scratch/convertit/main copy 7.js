...
const sheet = css`
  frame-component nav[slot="side"] {
    display: flex;
    flex-direction: column;
    font-size: 1.125rem;
  }

  frame-component nav[slot="top"] {
    display: flex;
  ...
  html`<form>
    <div class="input-group">
      ...
      <select
        ...
        title="Property"
      ></select>
      <span class="input-group-text" property-icon>icon</span>
    </div>
...
const propertyIcon = form.find(`span[property-icon]`);
const elements = form.elements;
elements.to_value.inert = true;
...
form.effects.add(
  (change, message) => {
    ...
    if (change._property) {
      ...
      const icon = icons[properties[change._property].icon];
      //console.log("icon:", icon); ////
      propertyIcon.clear();
      propertyIcon.innerHTML = icon;
      ...
      form.$({ _from_unit: elements.from_unit, _to_unit: elements.to_unit });
  ...
  ["_property", "_from_unit", "_from_value", "_to_unit"],
);
